...
const reducer = (state = initialState, action) => {
  if (action.type === "SET_TITULAR") {
    return {
      ...state,
      headliners: state.headliners.concat(action.players),
      players: state.players.filter((item) => item.id !== action.players.id),
    };
  }

  if (action.type === "ERASE_PLAYER") {
    console.log("player id: ", action.player.id);
    return {
      ...state,
      headliners: state.headliners.filter(
        (item) => item.id !== action.player.id
      ),
  ...
  if (action.type === "SET_SUBTITUTE") {
    return {
      ...state,
      substitutes: state.substitutes.concat(action.players),
      players: state.players.filter((item) => item.id !== action.players.id),
    };
  }
